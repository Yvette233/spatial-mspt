...
from utils.loss import TILDE_Q, TILDEQ_LOSS_OFFICIAL
from utils.featuremap2d import TimesNetFeatureMapVisual
import torch
...
class Exp_Main(Exp_Basic):
    ...
    def _select_scheduler(self, optimizer, train_steps):
        if self.args.lradj == 'ExponentialLR':
            ...
            return torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.000001, max_lr=self.args.learning_rate, step_size_up=train_steps, step_size_down=train_steps, mode='triangular2', gamma=1.0, scale_fn=None, scale_mode='cycle', cycle_momentum=True, base_momentum=0.8, max_momentum=0.9, last_epoch=-1, verbose=False)
            ...
            return torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=self.args.learning_rate, steps_per_epoch=train_steps, epochs=self.args.train_epochs)

    def _select_criterion(self):
        criterion = nn.MSELoss()
        # criterion = TILDEQ_LOSS_OFFICIAL(alpha=0.8, gamma=0.01)
        return criterion
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    # outputs = outputs[:, -self.args.pred_len:1-self.args.pred_len, f_dim:]
                    # batch_y = batch_y[:, -self.args.pred_len:1-self.args.pred_len, f_dim:].to(self.device)
                    if isinstance(criterion, TILDEQ_LOSS_OFFICIAL):
            ...
            if self.args.lradj != "OneCycleLR" and self.args.lradj != "ReduceLR":
                scheduler.step()  
    ...
    def predict_recurrent(self, model, batch_x, batch_x_mark, dec_inp, batch_y_mark):
        y = torch.zeros(dec_inp.shape[0], self.pred_len, dec_inp.shape[2], device=dec_inp.device)
        # print(y.shape)
        for i in range(self.pred_len):
            x = torch.cat((batch_x[:, i:, :], y[:, :i, :]), dim=1)
            yi = model(x, batch_x_mark, dec_inp, batch_y_mark)
            # print(x)
            y[:, i:i+1, :] = yi
        return y
    ...
    def test(self, setting, test=0):
        # set prediction length back to original
        self.args.pred_len = self.pred_len
        test_data, test_loader = self._get_data(flag='test')
        if test:
            print('loading model')
            self.model.load_state_dict(torch.load(os.path.join('/root/autodl-tmp/checkpoints/' + setting, 'checkpoint.pth'), map_location="cuda:0"))

        preds = []
        trues = []
        folder_path = '/root/autodl-tmp/test_results/' + setting + '/'
        ...
        # test_model = TimesNetFeatureMapVisual(setting, self.model, self.args.top_k, self.args.e_layers, self.args.d_layers, len(test_data))
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs = self.predict_recurrent(self.model, batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                        else:
                            outputs = self.predict_recurrent(test_model, batch_x, batch_x_mark, dec_inp, batch_y_mark)
                else:
                    if self.args.output_attention:
                        outputs = self.predict_recurrent(self.model, batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                ...
                if i % 20 == 0:
                    ...
                    gt = np.concatenate((input[0, -100:, -1], true[0, :, -1]), axis=0)
                    pd = np.concatenate((input[0, -100:, -1], pred[0, :, -1]), axis=0)
