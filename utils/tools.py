...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if args.lradj == 'type1':
        ...
        rate_list = [
            3, 5, 10, 30, 70, 100, 80, 40, 15, 8, 4, 1, 0.5, 0.1, 0.05, 0.01,
            0.005, 0.001, 0.0005, 0.0001
        ]
        lr_adjust = {epoch: args.learning_rate * rate_list[epoch - 1]}
    elif args.lradj == 'type7':
        rate_list = [
            3, 5, 10, 30, 30, 30, 30, 30, 15, 8, 4, 1, 0.5, 0.1, 0.05, 0.01,
            0.005, 0.001, 0.0005, 0.0001
        ]
        lr_adjust = {epoch: args.learning_rate * rate_list[epoch - 1]}
    ...
    def __call__(self, cur_step, cur_epoch, loss):
        ...
        if (cur_step + 1) % 100 == 0:
            print("\titers: {0}, epoch: {1} | loss: {2:.7f}".format(
                cur_step + 1, cur_epoch + 1, loss.item()))
            speed = (time.time() - self.time_now) / self.iter_count
            left_time = speed * (
                (self.num_epochs - cur_epoch) * self.num_steps - cur_step)
