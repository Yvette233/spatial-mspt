...
class VariablePatchEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        self.value_embedding_weight = nn.Parameter(
            self.scale * torch.randn(max_patch_len * e_in, d_model))
    ...
    def forward(self, x, period):
        ...
        x = x @ self.value_embedding_weight[:period * self.
                                            e_in, :] + self.value_embedding_bias
        # position embedding
        x = x + self.position_embedding(
            x)  # [B*N, T//period, D] or [B, T//period, D]
        return self.dropout(x)
...
def Coord2dPosEncoding(q_len,
    ...
    for i in range(100):
        cpe = 2 * (torch.linspace(0, 1, q_len).reshape(-1, 1)**
                   x) * (torch.linspace(0, 1, d_model).reshape(1, -1)**x) - 1
...
def positional_encoding(pe, learn_pe, q_len, d_model):
    ...
    if pe == None:
        ...
        W_pos = Coord2dPosEncoding(q_len,
                                   d_model,
                                   exponential=False,
                                   normalize=True)
    elif pe == 'exp2d':
