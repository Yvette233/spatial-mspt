...
class LinearPredictionHead(nn.Module):
    ...
    def forward(self, xs, gates, x_dec, x_mark_dec=None):
        # Ps*[B, C, L, D]
        for i, patch_size in enumerate(self.patch_sizes):
            xs[i] = self.linears[i](self.dropout(xs[i].flatten(start_dim=1))) # [B, C, L, D] -> [B, C, P]
...
class DecoderPredictionHead(nn.Module):
    ...
    def forward(self, xs, gates, x_dec, x_mark_dec=None):
        # Ps*[B, C, L, D]
        B, S, C = x_dec.shape
        x_dec = rearrange(x_dec, 'B S (C D) -> B C S D', D=1)
        x_dec = self.data_embedding(x_dec, None) # [B, C, S, D]
        x_decs = self.dispatcher(x_dec, gates) # Ps*[B, C, S, D]
        dec_outs = []
        for i, patch_size in enumerate(self.patch_sizes):
            x_ = rearrange(x_decs[i], 'B C S D -> (B C) S D')
            xs[i] = rearrange(xs[i], 'B C L D -> (B C) L D')
            dec_out = self.decoders[i](x_, xs[i]).squeeze(-1) # [B, C, S]
            dec_outs.append(dec_out)
        dec_outs = self.cspfa(dec_outs, gates) # [B, C, S]
        dec_outs = rearrange(dec_outs, 'B C S -> B S C') # [B, S, C]
...
class DecoderPretrainHead(nn.Module):
    def __init__(self, configs, patch_sizes):
        super(LinearPretrainHead, self).__init__()
        self.seq_len = configs.seq_len
        self.patch_sizes = patch_sizes
        self.mask_tokens = nn.Parameter(torch.zeros(len(patch_sizes), 1, 1, 1, configs.d_model))
        ...
        for patch_size in patch_sizes:
            ...
            self.decoder_preds.append(nn.Linear(configs.d_model, patch_size))
        self.cspfa = CrossScalePeriodicFeatureAggregator(patch_sizes, configs.seq_len, configs.d_model)
    
    def forward(self, xs, gates, ids_restores):
        # Ps*[B, C, L, D]
        B, C, L, D = xs.shape

        for i, patch_size in enumerate(self.patch_sizes):
            mask_token = self.mask_tokens[i, :, :, :, :].repeat(xs.shape[0], xs.shape[1], ids_restores[i].shape[1] + 1 - xs.shape[1], 1)
            x_ = torch.cat((xs[i], mask_token), dim=-2)
            x_ = torch.gather(x_, dim=-2, index=ids_restores[i].unsqueeze(-1).repeat(1, 1, 1, xs.shape[3]))
            x_ = self.dropout(x_ + self.pos_embedding(x_))
            xs[i], _ = self.decoders[i](x_)
            xs[i] = self.decoder_preds[i](xs[i])
            xs[i] = rearrange(xs[i], 'B C L P -> B (L P) C', P=patch_size)[:,:self.seq_len,:] # [B, L, C]
        xs = self.cspfa(xs, gates) # [B, L, C]
        return xs # [bs, L, C]
                   ...
                   [
                        CrossDimensionalPeriodicEncoderLayer(
                            CrossDimensionalPeriodicAttentionLayer(
                                FullAttention(False, configs.factor, attention_dropout=configs.dropout,
                                            output_attention=configs.output_attention), configs.d_model, configs.n_heads, configs.enc_in),
                            configs.d_model,
                            configs.d_ff,
                            dropout=configs.dropout,
                            activation=configs.activation
