...
class MultiScalePeriodicPatchEmbedding(nn.Module):
    def __init__(self, seq_len, top_k=5, d_model=512, dropout=0., sparsity_threshold=0.01, hidden_size_factor=4, c_in=14, individual=False):
        ...
        for patch_size in self.patch_sizes:
            self.value_embeddings.append(nn.Linear(patch_size*d_model, d_model, bias=False) if individual else nn.Linear(c_in*patch_size*d_model, d_model, bias=False))
    ...
    def afno1d_for_peroid_weights(self, x):
        ...
        xf = torch.fft.rfft(x, dim=-1) # [B, C, D, L//2+1]
        xf_no_zero = xf[:, :, 1:] # [B, C, D, L//2]

        o1_real = torch.zeros([B, C, D, self.freq_seq_len * self.hidden_size_factor], device=x.device)
        o1_imag = torch.zeros([B, C, D, self.freq_seq_len * self.hidden_size_factor], device=x.device)
        o2_real = torch.zeros([B, C, D, len(self.patch_sizes)], device=x.device)
        o2_imag = torch.zeros([B, C, D, len(self.patch_sizes)], device=x.device)
        ...
        xf_no_zero = torch.stack([o2_real, o2_imag], dim=-1) # [B, C, D, L-1, 2]
        xf_no_zero = F.softshrink(xf_no_zero, lambd=self.sparsity_threshold) # [B, C, D, L-1, 2]
        xf_no_zero = torch.view_as_complex(xf_no_zero) # [B, C, D, L-1]

        weights = torch.abs(xf_no_zero).mean(dim=-2).mean(dim=-2) # [B, L-1]
    ...
    def patch_embedding(self, x, patch_size, index_of_patch):
        B, L, C, D = x.shape
        # do patching
        x = rearrange(x, 'B L C D -> (B C) D L') if self.individual else rearrange(x, 'B L C D -> B (C D) L')
        x = self.padding_patch_layers[index_of_patch](x)
        x = x.unfold(-1, patch_size, patch_size) # [B, D, L//patch_size, patch_size]
        x = rearrange(x, 'B D L P -> B L (P D)')
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        B, L, C = x_enc.shape
        x_enc = self.start_fc(x_enc.unsqueeze(-1)) # [B, L, C, D]
