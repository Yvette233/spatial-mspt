...
class MultiScalePeriodicPatchEmbedding(nn.Module):
    def __init__(self, seq_len, top_k=5, d_model=512, dropout=0., sparsity_threshold=0.01, hidden_size_factor=4):
        ...
        # FFT parameters
        self.start_fc = nn.Linear(seq_len, seq_len)
        # Noise parameters
        self.w_noise = nn.Parameter(torch.zeros(seq_len, seq_len//2))
    ...
    def afft_for_peroid_weights(self, x, training=True, noise_epsilon=1e-2):
        # x [B, L, C]
        B, L, C = x.shape

        # start_fc
        x = self.start_fc(x.transpose(1, 2)).transpose(1, 2) # [B, L, C]

        xf = torch.fft.rfft(x, dim=-2, norm='ortho') # [B, L//2+1, C]

        amplitude_list = torch.abs(xf) # [B, L//2+1, C]
...
class CrossScalePeriodicFeatureAggregator(nn.Module):
    def __init__(self, patch_sizes, seq_len):
