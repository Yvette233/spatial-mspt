...
class EncoderStack(nn.Module):
    def __init__(self, configs):
        ...
        self.w_gamma = nn.Parameter(torch.zeros(int(configs.seq_len / 2), self.num_patch_sizes), requires_grad=True)
        self.b_gamma = nn.Parameter(torch.zeros(self.num_patch_sizes), requires_grad=True)
        self.w_noise = nn.Parameter(torch.zeros(int(configs.seq_len / 2), self.num_patch_sizes), requires_grad=True)        
    ...
    def get_patch_sizes(self, seq_len, exclude_zero=True):
        ...
        patch_sizes = peroid_list.int().unique().flip(0)
        return patch_sizes
    ...
    def fft_for_peroid(self, x, exclude_zero=True):
        ...
        return peroid_list, amplitude_list # /test1
        return peroid_list, (amplitude_list - amplitude_list.mean(-1, keepdim=True)) / amplitude_list.std(-1, keepdim=True) # /test2

    def forward(self, x, noise_epsilon=1e-2):
        # [B, L, C] or [B*C, L, 1]
        period_list, amplitude_list = self.fft_for_peroid(x) # [L], [B, L] or [B*C, L]
        period_weights = amplitude_list @ self.w_gamma + self.b_gamma # [B, num_period] or [B*C, num_period]
        clean_logits = period_weights # [B, num_period] or [B*C, num_period]
        if self.training:
            raw_noise_stddev = amplitude_list @ self.w_noise # [B, num_period] or [B*C, num_period]
            noise_stddev = ((F.softplus(raw_noise_stddev) + noise_epsilon)) # [B, num_period] or [B*C, num_period]
            noisy_logits = clean_logits + (torch.randn_like(clean_logits) * noise_stddev) # [B, num_period] or [B*C, num_period]
            logits = noisy_logits # [B, num_period] or [B*C, num_period]
        else:
            logits = clean_logits # [B, num_period] or [B*C, num_period]
        top_k_logits, top_k_indices = logits.topk(self.k, dim=-1) # [B, k] or [B*C, k]
        ...
        # sns.heatmap(gates.cpu().detach().numpy(), cmap='jet')
        # plt.savefig('/root/MSPT/test_visuals/gates.png')
        # fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(10, 10))
        # sns.heatmap(gates.cpu().detach().numpy(), cmap='jet', ax=ax[0, 0], label='gates')
        # sns.heatmap(clean_logits.cpu().detach().numpy(), cmap='jet', ax=ax[0, 1], label='clean_logits')
        # sns.heatmap(raw_noise_stddev.cpu().detach().numpy(), cmap='jet', ax=ax[1, 0], label='raw_noise_stddev')
        # sns.heatmap(noisy_logits.cpu().detach().numpy(), cmap='jet', ax=ax[1, 1], label='noisy_logits')
