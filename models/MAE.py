...
from utils.masking import SegmentWiseMasking
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        if self.head_type == "pretrain":
            ...
            segmentwise_masking = SegmentWiseMasking(self.seq_len, self.mask_ratio, device=enc_out.device)
            # print(segmentwise_masking.mask.int(), segmentwise_masking.mask.float().mean())
            mask = segmentwise_masking.mask.unsqueeze(0).repeat(enc_out.shape[0], 1)
            # print(enc_out * mask.unsqueeze(-1))
            enc_out, _ = self.encoder(enc_out * mask.unsqueeze(-1))
            enc_out = self.encoder_to_decoder(enc_out)
            dec_out = self.head(enc_out, mask, x_mask=None)
